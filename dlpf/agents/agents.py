...
class FlatAgentWithLossLogging(object):
    def __init__(self, state_size=None, number_of_actions=1,
                 epsilon=0.1, mbsz=32, discount=0.9, memory=100,
                 save_name='basic', save_freq=10):
        self.state_size = state_size
    ...
    def generate_batches(self, batch_size, width_of_layers=21, height_of_layers=21, mode='train'):
        input_batch = numpy.zeros((batch_size, width_of_layers, height_of_layers),
                                  dtype = 'float32')
        output_batch = numpy.zeros((batch_size, self.number_of_actions),
                                   dtype = 'float32')
        ...
        if len(inputs) == len(outputs):
            if mode == 'train':
                unused = set(range(int(len(outputs)*0.9)))
            else:
                unused = set(range(int(len(outputs)*0.9), len(outputs)))
            while len(unused) > 0:
                batch_indexes = rand.sample(unused, batch_size)
                #for i in batch_indexes:
                #    unused.remove(i)
    ...
    def train_with_full_experience(self, output=2, batch=100):
        batches = self.generate_batches(batch_size=batch,
                                        width_of_layers=2*self.vision_range+1,
                                        height_of_layers=2*self.vision_range+1)
        val_batches = self.generate_batches(batch_size=batch,
                                            width_of_layers=2*self.vision_range+1,
                                            height_of_layers=2*self.vision_range+1,
                                            mode='val')
        self.model.fit_generator(batches,
                                 ...
                                 nb_val_samples=len(self.states),
